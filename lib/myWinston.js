...
function createLoggerFor(labelName) {
    return createLogger({
        ...
        format: combine(json(), label({ label: labelName }), timestamp({ format: "YYYY-MM-DD HH:mm:ss" }), myFormat),
        transports: [
            new transports.File({
                level: "debug",
            ...
            new transports.Console({
                ...
                json: false,
                colorize: true,
                format: combine(colorize(), label({ label: labelName }), timestamp({ format: "YYYY-MM-DD HH:mm:ss" }), myFormat),
            }),
        ],
        exitOnError: false,
    });
