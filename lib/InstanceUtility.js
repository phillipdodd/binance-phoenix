...
class InstanceUtility {
    ...
    async cancelOrder(symbol, orderId) {
        try {
            this.instance.logger.error(`cancelOrder: attempting ${symbol} ${orderId}`);
            return await this.instance.client.cancelOrder({ symbol, orderId });
        } catch (err) {
            this.instance.logger.error(`cancelOrder: ${err.message}`);
            throw err;
        }
    }
    ...
    //todo this is the same thing as getBuyQuantity -- consolidate
    async getSellQuantity(symbol, lowestAskPrice) {
        try {
            const pairType = this.getPairType(symbol);
            const startingValue = this.instance.strategy[`starting${pairType}_SELL`];
            const buyQuantity = Calc.div(startingValue, lowestAskPrice);
            return buyQuantity;
        } catch (err) {
            this.instance.logger.error(`getBuyQuantity: ${err.message}`);
            console.error(err.message);
    ...
    async getBuyPrice(executionReport) {
        try {
            const tickDecrease = this.getTickDecreasePrice(executionReport);
            const highestBid = await this.getBestBidPrice(executionReport.symbol);
            return Math.min(+tickDecrease, +highestBid);
        } catch (err) {
            this.instance.logger.error(`getSellPrice: ${err.message}`);
            throw err;
        }
    ...
    getTickIncreasePrice(executionReport) {
        ...
        return Calc.add(buyPrice, increaseAmount);
    }

    getTickDecreasePrice(executionReport) {
        let tickSize = this.exchangeInfo[executionReport.symbol].tickSize;
        let decreaseAmount = Calc.mul(tickSize, this.instance.strategy.numTickDecrease);
        let sellPrice = getPriceValue(executionReport);
        return Calc.sub(sellPrice, decreaseAmount);
    }
    ...
    async isOrderFilled(symbol, orderId) {
        try {
            const order = await this.instance.client.getOrder({ symbol, orderId }).catch((err) => {
                throw err;
            });
            this.instance.logger.debug(`isOrderFilled status is ${order.status}`);
            return order.status === "FILLED";
        } catch (err) {
            this.instance.logger.error(`isOrderFilled: ${err.message}`);
            throw err;
        }
    }
}
