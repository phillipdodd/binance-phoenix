...
const { config } = require('../data/constants.js');
class Calc {
    static add(x, y, toFixedValue) {
        try {
            toFixedValue = toFixedValue ? toFixedValue : config.Calc.toFixedValue;
    ...
    static getAbsDiff(x, y, toFixedValue) {
        try {
            toFixedValue = toFixedValue ? toFixedValue : config.Calc.toFixedValue;
    ...
    static increaseByPercentage(decimal, percentage, toFixedValue) {
        try {
            toFixedValue = toFixedValue ? toFixedValue : config.Calc.toFixedValue;
    ...
    static decreaseByPercentage(decimal, percentage, toFixedValue) {
        try {
            toFixedValue = toFixedValue ? toFixedValue : config.Calc.toFixedValue;
            [decimal, percentage] = this.convertToBigNumbers(decimal, percentage);
